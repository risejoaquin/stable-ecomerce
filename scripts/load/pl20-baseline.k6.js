...
// PL20-03 SAFE_READ baseline draft.
// PREPARATION ONLY: do not run without explicit ChatGPT Web/user approval.
//
// Excluded on purpose:
// - checkout/payments/refunds: may create Stripe side effects.
// - orders/inventory: may mutate production data or stock.
// - email/newsletter/contact: may send or queue messages.
// - admin: may expose privileged data or trigger mutations.
// - analytics/ads/webhooks: may create tracking/provider events.
...
function requiredEnv(name) {
  ...
  if (value === undefined || String(value).trim() === '') {
    throw new Error(`${name} is required. This PL20 draft intentionally has no execution defaults.`);
  }
...
function assertValidLookingDuration(value) {
  if (!/^\d+(ms|s|m|h)$/.test(value)) {
    throw new Error('APPROVED_DURATION must be a non-empty k6-style duration such as 30s, 5m, or 1h.');
  }
  return value;
}
...
function assertApprovedSafeReadTargets(paths) {
  for (const path of paths) {
    if (!approvedSafeReadPaths.has(path)) {
      throw new Error(`Unapproved target path in PL20 k6 draft: ${path}`);
    }
  }
}
...
export const options = {
  scenarios: {
    pl20_baseline_placeholder: {
      executor: 'constant-vus',
      vus: approvedVus,
      duration: approvedDuration,
    },
  },
  // Thresholds are intentionally omitted. PL20-03 thresholds must be approved
  // from baseline evidence, product SLOs, or provider-plan limits.
};
...
export function handleSummary(data) {
  const outputPath = __ENV.K6_SUMMARY_PATH || 'pl20-baseline-summary.json';
  return {
    [outputPath]: JSON.stringify(data, null, 2),
    stdout: JSON.stringify({
      note: 'PL20 baseline draft completed. Do not treat as approved evidence unless the run was explicitly authorized.',
      ...
      metadata: {
        ...
        base_url: baseUrl,
        allow_production_load_test: __ENV.ALLOW_PRODUCTION_LOAD_TEST === 'true',
        execution_authorized: false,
        authorization_note: 'ALLOW_PRODUCTION_LOAD_TEST only prevents accidental production blocking; it does not authorize execution.',
      },
    }, null, 2),
  };
}
