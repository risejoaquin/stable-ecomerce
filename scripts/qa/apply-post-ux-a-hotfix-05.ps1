...
if (
  ...
  Write-Host "PASS POST-UX A HOTFIX 05 applied" -ForegroundColor Green
  exit 0
}
...
# IMPORTANT: single-quoted regex patterns prevent PowerShell from expanding
# $auditOutput, $auditExit, and $LASTEXITCODE while the regex is constructed.
$patterns = @(
...
if (-not $patched) {
  ...
  throw "Patch anchor not found in smoke-post-ux-a.ps1 even with interpolation-safe matcher"
}

[System.IO.File]::WriteAllText($full, $content, $utf8NoBom)

Write-Host "PATCH POST-UX A smoke uses explicit npm.cmd (interpolation-safe matcher)" -ForegroundColor Green
Write-Host "PASS POST-UX A HOTFIX 05 applied" -ForegroundColor Green
