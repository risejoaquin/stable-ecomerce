...
export function getMonthPeriodBounds(periodStr: string): { periodStart: string; periodEnd: string } {
  ...
  if (!match) {
    return { periodStart: `${periodStr}-01`, periodEnd: `${periodStr}-01` };
  }
  ...
  // Date.UTC(year, month, 0) gives the last day of the calendar month (inclusive)
  const lastDay = new Date(Date.UTC(year, month, 0)).getUTCDate();
  const padMonth = String(month).padStart(2, '0');
  const padLastDay = String(lastDay).padStart(2, '0');
  return {
    periodStart: `${year}-${padMonth}-01`,
    periodEnd: `${year}-${padMonth}-${padLastDay}`
  };
}
...
app.post(
  ...
  // ---------------------------------------------------------------------------
  // ---------------------------------------------------------------------------
  // POST-LAUNCH 20 — Final Commercial Scale Report & Strategic Roadmap (Evidence-Driven)
  // ---------------------------------------------------------------------------
  ...
  const validateFinalScaleKey = (key: any, fieldName = 'key'): string => {
    if (!key || typeof key !== 'string' || !/^[a-zA-Z0-9_-]{1,64}$/.test(key.trim())) {
      throw new AppError(`Invalid ${fieldName}: must be 1-64 alphanumeric characters, dashes or underscores`, 400);
    }
    return key.trim();
  };

  const validateFinalScalePeriod = (period: any): string => {
    if (!period || typeof period !== 'string' || !/^\d{4}-(0[1-9]|1[0-2])$/.test(period.trim())) {
      throw new AppError('Invalid period format: expected YYYY-MM', 400);
    }
    return period.trim();
  };

  const validateFinalScaleCurrency = (currency: any): string => {
    if (!currency || typeof currency !== 'string' || !/^[A-Za-z]{3}$/.test(currency.trim())) {
      throw new AppError('Invalid currency format: expected 3-letter ISO code (e.g. USD)', 400);
    }
    return currency.trim().toUpperCase();
  };

  const validateFinalScaleCostNumber = (val: any, fieldName: string): number => {
    if (val === undefined || val === null || val === '') return 0;
    const num = Number(val);
    if (!Number.isFinite(num) || num < 0) {
      throw new AppError(`Invalid ${fieldName}: must be a finite number >= 0`, 400);
    }
    return num;
  };
  ...
  app.get('/api/admin/final-scale/summary', requireAuth(), requireAdmin(), asyncHandler(async (req: any, res) => {
    ...
    for (const conf of requiredDimensionConfigs) {
      ...
      const origin = ev?.origin || meta?.origin || item?.origin || null;

      // Task 4: Defense in depth - summary must not trust classification string alone.
      // A row claiming classification=VERIFIED_CI_EVIDENCE must also carry trusted-origin metadata.
      // origin must be explicitly 'persisted_trusted_import'.
      // If trusted-origin marker is absent, or origin is 'request_body': downgrade to MANUAL_EVIDENCE.
      if (classification === 'VERIFIED_CI_EVIDENCE') {
        const isTrustedOrigin = origin === 'persisted_trusted_import';
      ...
      // For database reproducibility: allowed classifications are ['VERIFIED_CI_EVIDENCE', 'PERSISTED_EVIDENCE']
      // If classification === 'PERSISTED_EVIDENCE', origin must be 'persisted_trusted_import' or 'persisted_database_evidence'.
      if (classification === 'PERSISTED_EVIDENCE') {
        const isTrustedDbOrigin = origin === 'persisted_trusted_import' || origin === 'persisted_database_evidence';
        if (!isTrustedDbOrigin) {
          classification = 'MANUAL_EVIDENCE';
        }
      }
    ...
    let secOrigin: string | null = null;
    ...
    if (secBlockerItem) {
      ...
      secOrigin = ev?.origin || meta?.origin || secBlockerItem.origin || null;
      ...
      // Task 5: Security blockers same rule.
      // A caller must not be able to self-label REVIEWED_SECURITY_EVIDENCE or VERIFIED_CI_EVIDENCE.
      // Origin must be server-trusted.
      if (rawClassification === 'VERIFIED_CI_EVIDENCE') {
        const hasFullProof = Boolean(
          secOrigin === 'persisted_trusted_import' &&
          secValidatedSha &&
        ...
        if (secOrigin !== 'persisted_trusted_import' && secOrigin !== 'persisted_database_evidence') {
          rawClassification = 'MANUAL_EVIDENCE';
        }
      } else {
        rawClassification = 'MANUAL_EVIDENCE';
    ...
    const isCiDimSatisfied = (conf: any) => {
      ...
      if (!conf.allowedClassifications.includes(dim.classification)) return false;
      const allowedOrigins = conf.id === 'database_reproducibility'
        ? ['persisted_trusted_import', 'persisted_database_evidence']
        : ['persisted_trusted_import'];
      return allowedOrigins.includes(dim.origin);
    };
    ...
    const isSecurityBlockersSatisfied = () => {
      ...
      const allowedClassifications = ['VERIFIED_CI_EVIDENCE', 'PERSISTED_EVIDENCE', 'REVIEWED_SECURITY_EVIDENCE'];
      if (!allowedClassifications.includes(sec.classification)) return false;
      const trustedOrigins = ['persisted_trusted_import', 'reviewed_security', 'persisted_database_evidence'];
      return trustedOrigins.includes(sec.origin);
    };
    ...
    res.json({
      ...
      summary: {
        ...
        evaluationRules: {
          ...
          technicalEvidenceCurrent,
          technicalRequiredPass,
          isSecurityBlockersSatisfied: isSecurityBlockersSatisfied(),
          technicalDimensions,
          currentCommitSha,
          hasCriticalTechnicalFailure,
          hasCriticalRisk,
  ...
  app.get('/api/admin/final-scale/technical-assessment', requireAuth(), requireAdmin(), asyncHandler(async (_req: any, res) => {
    res.json({ status: 'ok', assessments: await getFinalScaleTable('final_technical_assessments', 500) });
  }));

  app.post('/api/admin/final-scale/technical-assessment/run', requireAuth(), requireAdmin(), asyncHandler(async (req: any, res) => {
    const rawKey = req.body?.runKey || req.body?.run_key || 'technical-assessment';
    ...
    const classifyCiDimension = (dim: any) => {
      // TASK 1, 2, 3: REQUEST BODY IS NEVER VERIFIED CI!
      // Any evidence coming directly from req.body (ciEvidence, dimensions) must be classified as
      // MANUAL_EVIDENCE (if status is pass/fail) or NOT_MEASURED.
      // It MUST NEVER become VERIFIED_CI_EVIDENCE regardless of whether the payload includes
      // workflow_name, run_id, evidence_reference, validated_commit_sha, measured_at, source_type.
      const origin = 'request_body';
      ...
      // Always MANUAL_EVIDENCE from request body (or NOT_MEASURED if status is not_measured)
      return {
        status,
        classification: status === 'not_measured' ? 'NOT_MEASURED' : 'MANUAL_EVIDENCE',
        origin,
        validatedCommitSha: String(validatedSha).trim(),
        measuredAt: String(measuredAtVal).trim(),
        sourceType: sourceTypeVal,
        evidenceReference: evidenceRefVal ? String(evidenceRefVal).trim() : null,
        workflowIdentity: workflowIdentityVal ? String(workflowIdentityVal).trim() : null
      };
    };
    ...
    const rows = [
      ...
      (() => {
        ...
        return {
          ...
          finding: parsed.status === 'pass'
            ? `Automated release gate claim received via admin request body for commit ${parsed.validatedCommitSha || 'unknown'}.`
      ...
      (() => {
        ...
        return {
          ...
          score: null,
          finding: parsed.status === 'pass'
            ? `Production smoke claim received via admin request body for commit ${parsed.validatedCommitSha || 'unknown'}.`
      ...
      (() => {
        ...
        return {
          ...
          score: null,
          finding: parsed.status === 'pass'
            ? `Production build claim received via admin request body for commit ${parsed.validatedCommitSha || 'unknown'}.`
      ...
      (() => {
        ...
        return {
          ...
          score: null,
          finding: parsed.status === 'pass'
            ? `Automated unit/API test claim received via admin request body for commit ${parsed.validatedCommitSha || 'unknown'}.`
      ...
      (() => {
        ...
        return {
          ...
          score: null,
          finding: parsed.status === 'pass'
            ? `Playwright browser E2E test claim received via admin request body for commit ${parsed.validatedCommitSha || 'unknown'}.`
      ...
      (() => {
        ...
        return {
          ...
          score: null,
          finding: parsed.status === 'pass'
            ? `Repository secret scan claim received via admin request body for commit ${parsed.validatedCommitSha || 'unknown'}.`
      ...
      (() => {
        ...
        return {
          ...
          score: null,
          finding: parsed.status === 'pass'
            ? 'Supabase remote schema baseline claim received via admin request body.'
      ...
      (() => {
        ...
        return {
          ...
          score: null,
          finding: openCount === 0
            ? 'Zero critical P0/P1 security blockers open (request body manual claim).'
      ...
      const rowOrigin = (row as any).origin || 'request_body';
      ...
      return {
        ...
        metadata: {
          source: workflowIdentity || (rowOrigin === 'runtime' ? 'system_probes' : 'api_final_scale_technical_assessment_run'),
          source_type: 'api',
          source_classification: row.source_classification || 'RUNTIME_OBSERVED',
          origin: rowOrigin,
          calculation_version: 'pl20-02-v1',
  ...
  app.get('/api/admin/final-scale/commercial-assessment', requireAuth(), requireAdmin(), asyncHandler(async (_req: any, res) => {
    res.json({ status: 'ok', assessments: await getFinalScaleTable('final_commercial_assessments', 500) });
  }));

  app.post('/api/admin/final-scale/commercial-assessment/run', requireAuth(), requireAdmin(), asyncHandler(async (req: any, res) => {
    const rawKey = req.body?.runKey || req.body?.run_key || 'commercial-assessment';
    const runKey = validateFinalScaleKey(rawKey, 'runKey');
    const storeId = await getPrimaryStoreId();
    ...
    const rows = [
      ...
      {
        assessment_key: 'growth_and_traffic_attribution',
        area: 'growth',
        status: 'not_measured',
        score: null,
        finding: 'Paid ad attribution and external multichannel marketing feeds are structurally implemented but not connected to live ad spend.',
        recommendation: 'Connect live advertising platforms (Meta Pixel, Google Ads) when ready.'
      },
      {
        ...
        recommendation: 'Review supplier fulfillment SLAs weekly.'
      }
    ].map((row) => ({
      store_id: storeId,
      ...row,
  ...
  app.get('/api/admin/final-scale/risk-matrix', requireAuth(), requireAdmin(), asyncHandler(async (_req: any, res) => {
    res.json({ status: 'ok', risks: await getFinalScaleTable('strategic_risk_matrix', 500) });
  }));

  app.post('/api/admin/final-scale/risk-matrix/run', requireAuth(), requireAdmin(), asyncHandler(async (req: any, res) => {
    ...
    const rows = [
      {
        risk_key: 'traffic_scale_requires_real_load_test',
        category: 'scale',
        severity: 'medium',
        probability: 'high',
        impact: 'high',
        status: 'open',
        description: 'Synthetic smoke checks do not measure multi-user concurrency limits under peak traffic.',
        mitigation: 'Conduct automated load tests before scaling marketing campaigns.'
      },
      {
        risk_key: 'external_channel_integrations_pending',
        category: 'integrations',
        severity: 'medium',
        probability: 'medium',
        impact: 'medium',
        status: 'open',
        description: 'External channels (social, marketplace) are structurally designed but require live merchant credentials.',
        mitigation: 'Roll out connector credentials per channel during phased expansion.'
      },
      {
        risk_key: 'stripe_live_mode_operational_readiness',
        category: 'payments',
        severity: 'low',
        probability: 'low',
        impact: 'high',
        status: 'managed',
        description: 'Stripe webhook and checkout are verified; live transaction monitoring required at launch.',
        mitigation: 'Monitor Stripe dashboard webhooks in real time during initial launch phase.'
      }
    ].map((row) => ({
      store_id: storeId,
      ...row,
    ...
    await writeAuditLog({ actorUserId: req.auth?.userId, action: 'final_scale_risk_matrix_run', entityType: 'strategic_risk_matrix', metadata: { count: data.length, runKey } });
    res.json({ status: 'ok', risks: data });
  }));

  app.get('/api/admin/final-scale/technical-debt', requireAuth(), requireAdmin(), asyncHandler(async (_req: any, res) => {
    res.json({ status: 'ok', debt: await getFinalScaleTable('technical_debt_matrix', 500) });
  }));

  app.post('/api/admin/final-scale/technical-debt/run', requireAuth(), requireAdmin(), asyncHandler(async (req: any, res) => {
    ...
    const rows = [
      {
        debt_key: 'monolithic_server_ts',
        area: 'backend',
        severity: 'low',
        status: 'managed',
        description: 'server.ts contains API routes, middleware, and admin handlers in a large single file.',
        business_impact: 'Developer velocity and maintainability.',
        remediation_plan: 'Extract route modules into dedicated express routers in future architectural refactor.',
        estimated_effort: 'medium'
      },
      {
        debt_key: 'lack_of_automated_load_tests',
        area: 'performance',
        severity: 'medium',
        status: 'open',
        description: 'Lack of automated load and stress testing harness.',
        business_impact: 'Traffic spike resilience under extreme concurrent load is unverified.',
        remediation_plan: 'Create k6 or Artillery load test suite before high-volume commercial scaling.',
        estimated_effort: 'medium'
      },
      {
        debt_key: 'remote_migration_history_catchup',
        area: 'database',
        severity: 'low',
        status: 'closed',
        description: 'Remote Supabase schema lacked migration history table.',
        business_impact: 'Resolved by synchronizing baseline migration 20260918004527_remote_schema.sql.',
        remediation_plan: 'Maintain all future schema updates as incremental Supabase migrations.',
        estimated_effort: 'closed'
      }
    ].map((row) => ({
      store_id: storeId,
      ...row,
    ...
    await writeAuditLog({ actorUserId: req.auth?.userId, action: 'final_scale_technical_debt_run', entityType: 'technical_debt_matrix', metadata: { count: data.length, runKey } });
    res.json({ status: 'ok', debt: data });
  }));

  app.get('/api/admin/final-scale/operating-costs', requireAuth(), requireAdmin(), asyncHandler(async (_req: any, res) => {
    res.json({ status: 'ok', costs: await getFinalScaleTable('operating_cost_summaries', 250) });
  }));

  app.post('/api/admin/final-scale/operating-costs/run', requireAuth(), requireAdmin(), asyncHandler(async (req: any, res) => {
    const storeId = await getPrimaryStoreId();
    ...
    const measuredAt = new Date().toISOString();
    const { periodStart: defaultPeriodStart, periodEnd: defaultPeriodEnd } = getMonthPeriodBounds(period);
    ...
    if (!hasExplicitProviders && !hasNumericEstimates && !useOperatorFacts) {
      const payload = {
        ...
        metadata: {
          ...
          currency,
          measured_provider_total: 0,
          partial_provider_amounts: {},
          partial_known_amounts: {},
          shared_account_costs: {},
          shared_unallocated_amounts: {},
          unallocated_providers: [],
          unknown_amount_providers: [],
          caveats: ['No provider cost evidence or estimates provided.'],
          providers: {
            railway: { provider: 'railway', amount: null, period_start: defaultPeriodStart, period_end: defaultPeriodEnd, measured_state: 'NOT_MEASURED' },
            supabase: { provider: 'supabase', amount: null, period_start: defaultPeriodStart, period_end: defaultPeriodEnd, measured_state: 'NOT_MEASURED' },
            stripe: { provider: 'stripe', amount: null, period_start: defaultPeriodStart, period_end: defaultPeriodEnd, measured_state: 'NOT_MEASURED' },
            resend: { provider: 'resend', amount: null, period_start: defaultPeriodStart, period_end: defaultPeriodEnd, measured_state: 'NOT_MEASURED' },
            email: { provider: 'resend', amount: null, period_start: defaultPeriodStart, period_end: defaultPeriodEnd, measured_state: 'NOT_MEASURED' }
          }
        },
        updated_at: measuredAt
      };
    ...
    // -------------------------------------------------------------------------
    // Provider Cost Contract Evaluation (Tasks 1, 2, 3, 4, 5, 6, 7)
    // -------------------------------------------------------------------------

    // 1. RAILWAY (Tasks 1, 4, 5, 6)
    ...
    const railwayPeriodStart = rawRailway?.period_start || rawRailway?.periodStart || defaultPeriodStart;
    const railwayPeriodEnd = rawRailway?.period_end || rawRailway?.periodEnd || defaultPeriodEnd;
    ...
    if (railwayAllocationModel === 'shared_unallocated') {
      // Task 1 & 4: Preserve account_total = 192, shared_hosts = 4, allocation_model = shared_unallocated, measured_state = PARTIAL.
      // But ecommerce attributable amount = null unless an approved allocation method exists.
      // Do NOT use 192 as ecommerce amount. Do NOT divide by 4 automatically.
      // Store 192 only as shared/account infrastructure evidence.
      railwayMeasuredState = 'PARTIAL';
      railwayAmount = null;
      railwayCaveats.push(`Total account infrastructure cost of ${railwayAccountTotal} MXN/month is shared across ${railwaySharedHosts} hosts; ecommerce-specific allocation is shared_unallocated (PARTIAL). Attributable amount is null. Do not automatically divide by 4 or record 48 MXN as actual cost.`);
      ...
      railwayMeasuredState = 'PARTIAL';
      railwayAmount = null;
      railwayCaveats.push(`Railway allocation method '${railwayAllocationModel}' is unapproved heuristic and remains PARTIAL; attributable amount is null.`);
    }
    ...
    const supabasePeriodStart = rawSupabase?.period_start || rawSupabase?.periodStart || defaultPeriodStart;
    const supabasePeriodEnd = rawSupabase?.period_end || rawSupabase?.periodEnd || defaultPeriodEnd;
    ...
    const stripePeriodStart = rawStripe?.period_start || rawStripe?.periodStart || defaultPeriodStart;
    const stripePeriodEnd = rawStripe?.period_end || rawStripe?.periodEnd || defaultPeriodEnd;
    ...
    const resendPeriodStart = rawResend?.period_start || rawResend?.periodStart || defaultPeriodStart;
    const resendPeriodEnd = rawResend?.period_end || rawResend?.periodEnd || defaultPeriodEnd;
    ...
    if (resendAmount === 0) {
      ...
      if (hasZeroProvenance) {
        ...
        if (!resendCaveats.some(c => /free\s*tier/i.test(c))) {
          resendCaveats.push('Free tier plan active with 0 MXN baseline cost.');
        }
      } else {
    ...
    // -------------------------------------------------------------------------
    // Total State Derivation (Tasks 1, 2, 3, 5)
    // -------------------------------------------------------------------------
    ...
    // 1. unallocated_providers
    const unallocatedProviders = providerRecords
      .filter((p: any) => p.allocation_model === 'shared_unallocated')
      .map(p => p.provider);

    // 2. unknown_amount_providers
    const unknownAmountProviders = providerRecords
    ...
    providerRecords.forEach(p => {
      if (p.measured_state === 'PARTIAL' && typeof p.amount === 'number') {
        partialProviderAmounts[p.provider] = p.amount;
      }
    });

    // 5. shared_account_costs / shared_unallocated_amounts: Shared infrastructure evidence
    const sharedAccountCosts: Record<string, number> = {};
    if (railwayAllocationModel === 'shared_unallocated' && typeof railwayAccountTotal === 'number') {
    ...
    if (unallocatedProviders.length > 0 || unknownAmountProviders.length > 0 || totalMeasuredState === 'PARTIAL') {
      allCaveats.push(
        `Cost aggregation is PARTIAL: unallocated providers [${unallocatedProviders.join(', ')}], unknown amount providers [${unknownAmountProviders.join(', ')}]. ` +
        `Shared account infrastructure costs are separated in shared_account_costs and do not enter ecommerce-attributable total. ` +
    ...
    const payload = {
      ...
      cost_key: costKey,
      period,
      railway_estimate: railwayRecord.amount,
      supabase_estimate: supabaseRecord.amount,
      ...
      metadata: {
        ...
        shared_account_costs: sharedAccountCosts,
        shared_unallocated_amounts: sharedAccountCosts,
    ...
    const data = await runFinalScaleUpsert('operating_cost_summaries', [payload], 'store_id,period,cost_key');
    await writeAuditLog({ actorUserId: actorId, action: 'final_scale_operating_costs_run', entityType: 'operating_cost_summaries', entityId: data[0]?.id, metadata: { period, costKey, totalEstimate: totalEstimateValue, measuredProviderTotal, measured_state: totalMeasuredState } });
  ...
  app.post('/api/admin/final-scale/capacity/run', requireAuth(), requireAdmin(), asyncHandler(async (req: any, res) => {
    const rawKey = req.body?.runKey || req.body?.run_key || 'capacity';
    const runKey = validateFinalScaleKey(rawKey, 'runKey');
    const storeId = await getPrimaryStoreId();
  ...
  app.post('/api/admin/final-scale/strategic-roadmap', requireAuth(), requireAdmin(), asyncHandler(async (req: any, res) => {
    if (!supabase) return res.json({ status: 'ok', item: null });
    const storeId = await getPrimaryStoreId();
    const rawKey = req.body?.roadmapKey || req.body?.roadmap_key || `roadmap-${Date.now()}`;
    const roadmapKey = validateFinalScaleKey(rawKey, 'roadmapKey');
    ...
    const validStatuses = ['planned', 'in_progress', 'completed', 'deferred', 'cancelled'];
    ...
    const payload = {
      ...
      phase: req.body?.phase || 'Roadmap 2.0',
      title: req.body?.title || 'Strategic roadmap item',
      objective: req.body?.objective || null,
      priority,
      status,
      target_quarter: req.body?.targetQuarter || req.body?.target_quarter || null,
  ...
  app.get('/api/admin/final-scale/scale-decision', requireAuth(), requireAdmin(), asyncHandler(async (_req: any, res) => {
    res.json({ status: 'ok', decisions: await getFinalScaleTable('scale_decision_records', 100) });
  }));

  app.post('/api/admin/final-scale/scale-decision', requireAuth(), requireAdmin(), asyncHandler(async (req: any, res) => {
    if (!supabase) return res.json({ status: 'ok', decision: null });
    const storeId = await getPrimaryStoreId();
    const rawKey = req.body?.decisionKey || req.body?.decision_key || `decision-${Date.now()}`;
    const decisionKey = validateFinalScaleKey(rawKey, 'decisionKey');

    const validDecisions = ['scale_aggressively', 'scale_carefully', 'pause_scaling', 'remediate_first'];
    const decision = req.body?.decision || 'scale_carefully';
    if (!validDecisions.includes(decision)) {
      throw new AppError(`Invalid decision: must be one of ${validDecisions.join(', ')}`, 400);
    }
    ...
    const payload = {
      ...
      decision,
      status,
      rationale: req.body?.rationale || 'PL20 evidence-driven scale decision.',
      conditions: req.body?.conditions || [],
  ...
  app.get('/api/admin/final-scale/investor-readiness', requireAuth(), requireAdmin(), asyncHandler(async (_req: any, res) => {
    res.json({ status: 'ok', checks: await getFinalScaleTable('investor_readiness_checks', 500) });
  }));

  app.post('/api/admin/final-scale/investor-readiness/run', requireAuth(), requireAdmin(), asyncHandler(async (req: any, res) => {
    ...
    const rows = [
      ...
      {
        check_key: 'commercial_track_record',
        category: 'commercial',
        status: 'warning',
        score: null,
